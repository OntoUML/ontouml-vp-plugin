...
public class ReloadClassesController implements VPActionController {
  ...
  private static Set<String> blacklistSet = Set.of(".git", ".vscode"
...
//      , "zanetti2019representacao"
  ...
  private void processRepository() throws IOException {
    if(!repositoryFile.isDirectory()) {
      throw new RuntimeException("Invalid repository directory: '" + repositoryFile.getAbsolutePath() + "'");
    }

    Files.newDirectoryStream(repositoryFile.toPath())
        .forEach(ontologyPath -> {
...
//          }
          ...
          try {
            ...
            e.printStackTrace();
            throw new RuntimeException("Unable to open ontology '" + ontologyDir.getName() + "'");
          }
          ...
          try {
            ...
            e.printStackTrace();
            throw new RuntimeException("Unable to export diagrams of '" + ontologyDir.getName() + "'");
          }
          ...
          try {
            ...
            e.printStackTrace();
            throw new RuntimeException("Unable to export JSON of '" + ontologyDir.getName() + "'");
          }
  ...
  private void emptyFolder(File diagramsDir) throws IOException {
    Files.newDirectoryStream(diagramsDir.toPath())
  ...
  private void checkOntologyDir(File ontologyDir) {
    ...
    File originalDiagramsDir = Paths.get(ontologyDir.getAbsolutePath(), "original diagrams").toFile();
    ...
    if(!ontologyVppFile.exists()) {
      throw new RuntimeException("Visual Paradigm file not found! (" + ontologyVppFile.getPath() + ")");
    }
    ...
    diagramsDir = shouldExportToOriginalDiagramsDir(originalDiagramsDir, newDiagramsDir) ?
        originalDiagramsDir : newDiagramsDir;
  }
  ...
  private File getDirectory() {
    ...
    JFileChooser jc = ontoumlModelsDir.isDirectory() ?
        new JFileChooser(ontoumlModelsDir) : new JFileChooser();
