...
public class ViewUtils {
  ...
  public static void cleanAndShowMessage(String message) {
    ...
    ApplicationManager.instance()
        .getViewManager()
        .showMessagePaneComponent(OntoUMLPlugin.PLUGIN_ID, ViewUtils.SCOPE_PLUGIN, parentContainer);
  }
  ...
  public static String setOrderDialog(String currentOrder) {
    ...
    return (String)
        vm.showInputDialog(
            vm.getRootFrame(), msg, title, JOptionPane.PLAIN_MESSAGE, icon, null, currentOrder);
  }
  ...
  public static void updateDialog() {
    ...
    builder.append(
        "Please select the ZIP file containing the release of the OntoUML Plguin that you wish to"
            + " install.<br>");
    builder.append(
        "The latest release is available at <a href=\""
            + OntoUMLPlugin.PLUGIN_REPO
            + "\">"
            + OntoUMLPlugin.PLUGIN_REPO
            + "</a>.<br><br>");
...
final class ContextMenu extends JPopupMenu {
  ...
  public ContextMenu(String idModelElement) {
    ...
    takeMeThere =
        new JMenuItem(
            "Take me there!", new ImageIcon(ViewUtils.getFilePath(ViewUtils.NAVIGATION_LOGO)));
    takeMeThere.addActionListener(menuListener);
    openSpec =
        new JMenuItem(
            "Open Specification", new ImageIcon(ViewUtils.getFilePath(ViewUtils.MORE_HORIZ_LOGO)));
