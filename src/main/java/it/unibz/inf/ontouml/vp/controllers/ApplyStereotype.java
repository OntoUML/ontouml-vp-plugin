...
public class ApplyStereotype implements VPContextActionController {
  ...
  public void update(VPAction action, VPContext context) {
    ...
    if (context.getModelElement().getModelType().equals(IModelElementFactory.MODEL_TYPE_CLASS)
    ...
    IDiagramElement[] diagramElements =
        dm.getActiveDiagram() != null ? dm.getActiveDiagram().getSelectedDiagramElement() : null;
    ...
    for (IDiagramElement diagramElement : diagramElements) {
      if (diagramElement
          .getModelElement()
          .getModelType()
          .equals(context.getModelElement().getModelType()))
  ...
  private void applyStereotype(VPAction action, IModelElement element) {
    ...
    if (isSmartModelingEnabled && isClass) SmartModelling.setClassMetaProperties((IClass) element);
  ...
  private boolean isClassSelectionAllowed() {
    ...
    if (diagramElements == null) return true;

    if (diagramElements.length == 1) return true;

    // build list of Ids of all classes selected
    for (IDiagramElement diagramElement : diagramElements) {

      ISimpleRelationship[] relationshipsTo =
          diagramElement.getModelElement().toToRelationshipArray();
      ...
      for (int i = 0; relationshipsTo != null && i < relationshipsTo.length; i++) {
        ...
        if (!(superClassType.equals(IModelElementFactory.MODEL_TYPE_CLASS))) continue;
    ...
    for (int i = 0; i < superClasses.size(); i++) {
      ...
      for (int j = 0; j < superClasses.size(); j++) {
        ...
        if (id.equals(idAux)) counter++;
      }

      if (counter == countClassesSelected()) return true;
      else counter = 0;
    }
  ...
  private int countClassesSelected() {
    ...
    if (diagramElements == null) return count;

    for (IDiagramElement diagramElement : diagramElements) {
      if (diagramElement
          .getModelElement()
          .getModelType()
          .equals(IModelElementFactory.MODEL_TYPE_CLASS)) count++;
    }
