...
public class DBExportServerRequest extends Thread{
    ...
    public void run() {
		...
		try {
			...
			final BufferedReader script = OntoUMLServerAccessController.transformToDB(
			...
			if (script != null) {
				...
				ViewManagerUtils.cleanAndShowMessage("Unable to generate the files. Please check your model.");
			}
    ...
    private void saveFile(BufferedReader buffer) throws IOException {
        ...
        String suggestedFileName = ApplicationManager.instance().getProjectManager().getProject().getName() + exportFormatExtension;
