...
public final class Class extends Classifier<Class, ClassStereotype> {
   ...
   public void setLiterals(Collection<Literal> literals) {
      if (literals == null)
         throw new NullPointerException("Cannot set a null literal list.");
   ...
   public List<OntoumlElement> getContents() {
      ...
      if (hasAttributes())
         OntoumlUtils.addIfNotNull(contents, getAttributes());

      if (hasLiterals())
         OntoumlUtils.addIfNotNull(contents, getLiterals());
   ...
   public boolean restrictedToEquals(List<Nature> natures) {
      ...
      if (restrictedTo.size() != natures.size())
         return false;
