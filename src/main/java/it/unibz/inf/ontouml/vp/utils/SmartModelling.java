...
public class SmartModelling {
    ...
    switch (stereotypes[0]) {
      ...
      case StereotypeUtils.STR_MATERIAL:
        if (targetStereotype.equals(StereotypeUtils.STR_ROLE)
            || targetStereotype.equals(StereotypeUtils.STR_ROLE_MIXIN))
          setCardinalityIfEmpty(source, "1..*");
        else setCardinalityIfEmpty(source, "0..*");

        if (sourceStereotype.equals(StereotypeUtils.STR_ROLE)
            || sourceStereotype.equals(StereotypeUtils.STR_ROLE_MIXIN))
          setCardinalityIfEmpty(target, "1..*");
  ...
  public static void manageAssociationStereotypes(IAssociation association, VPAction action) {
    ...
    if (!association.getFrom().getModelType().equals(IModelElementFactory.MODEL_TYPE_CLASS)) return;

    if (!association.getTo().getModelType().equals(IModelElementFactory.MODEL_TYPE_CLASS)) return;

    final IClass source = (IClass) association.getFrom();
    final IClass target = (IClass) association.getTo();

    final ArrayList<String> sourceStereotypes =
        new ArrayList<String>(Arrays.asList(source.toStereotypeArray()));
    final ArrayList<String> targetStereotypes =
        new ArrayList<String>(Arrays.asList(target.toStereotypeArray()));
    ...
    final List<String> allowedCombinations =
        AssociationConstraints.getAllowedActionIDs(sourceStereotype, targetStereotype);
  ...
  public static void manageClassStereotypes(IClass _class, VPAction action) {
    ...
    for (int i = 0; relationshipsTo != null && i < relationshipsTo.length; i++) {
      ...
      final String superClassType =
          relationshipTo.getFrom() != null ? relationshipTo.getFrom().getModelType() : "";

      if (!(relationshipTypeTo.equals(IModelElementFactory.MODEL_TYPE_GENERALIZATION))
          || !(superClassType.equals(IModelElementFactory.MODEL_TYPE_CLASS))) continue;

      final IClass superClass = (IClass) relationshipTo.getFrom();
      final ArrayList<String> superClassStereotypes =
          new ArrayList<String>(Arrays.asList(superClass.toStereotypeArray()));

      if (superClassStereotypes.size() == 0) continue;

      if (superClassStereotypes.size() > 1) action.setEnabled(false);

      final String superStereotype = superClassStereotypes.get(0);
      final List<String> allowedCombinationsSub =
          ClassConstraints.getAllowedActionIDsOnGeneral(superStereotype);
    ...
    for (int i = 0; relationshipsFrom != null && i < relationshipsFrom.length; i++) {
      ...
      final String subClassType =
          relationshipFrom.getTo() != null ? relationshipFrom.getFrom().getModelType() : "";

      if (!(relationshipTypeFrom.equals(IModelElementFactory.MODEL_TYPE_GENERALIZATION))
          || !(subClassType.equals(IModelElementFactory.MODEL_TYPE_CLASS))) continue;

      final IClass subClass = (IClass) relationshipFrom.getTo();
      final ArrayList<String> subClassStereotypes =
          new ArrayList<String>(Arrays.asList(subClass.toStereotypeArray()));

      if (subClassStereotypes.size() == 0) continue;

      if (subClassStereotypes.size() > 1) action.setEnabled(false);

      final String subStereotype = subClassStereotypes.get(0);
      final List<String> allowedCombinationsSuper =
          ClassConstraints.getAllowedActionIDsOnSpecific(subStereotype);

      if (allowedCombinationsSuper == null
          || !allowedCombinationsSuper.contains(action.getActionId())) action.setEnabled(false);
    }
