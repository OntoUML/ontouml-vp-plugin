...
public class Model implements ModelElement {
  ...
  /**
   * Constructs a model to contain all project's model elements independent of a <code>IModelElement
   * </code>.
   */
  public Model() {
  ...
  public Model(HashSet<String> idElements) {
    ...
    if (containsDiagramModel(idElements)) {
      ...
      while (ite.hasNext()) {
        ...
        if (project.getModelElementById(id) == null) continue;
        else modelElements.add(project.getModelElementById(id));
      }
  ...
  /**
   * Constructs a model based on a <code>IModelElement</code> and which is serialized as a Package
   * in OntoUML Schema.
   */
  ...
  public void addElement(ModelElement element) {
    if (this.contents == null) this.contents = new ArrayList<ModelElement>();
  ...
  private void addModelElements(IModelElement[] modelElements, HashSet<String> idElements) {
    for (int i = 0; modelElements != null && i < modelElements.length; i++) {
      ...
      if (!idElements.contains(projectElement.getId())) continue;
  ...
  private static boolean containsDiagramModel(HashSet<String> idElements) {
    ...
    while (ite.hasNext()) {
      ...
      if (project.getDiagramById(id) == null) continue;
      else return true;
    }
