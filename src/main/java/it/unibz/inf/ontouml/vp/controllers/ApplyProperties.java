...
public class ApplyProperties implements VPContextActionController {
  ...
  private void forEachSelectedClass(VPContext context, Consumer<IClass> consumer) {
    if (!(context.getModelElement() instanceof IClass)) return;
    ...
    Arrays.stream(diagramElements)
        .filter(
            e -> e.getModelElement().getModelType().equals(IModelElementFactory.MODEL_TYPE_CLASS))
  ...
  private void setBooleanTaggedValue(VPContext context, IClass clickedClass, String metaProperty) {
    final ITaggedValue booleanTaggedValue =
        StereotypeUtils.reapplyStereotypeAndGetTaggedValue(clickedClass, metaProperty);
    final boolean value =
    ...
    forEachSelectedClass(
        ...
        cla -> {
          ITaggedValue taggedValue =
              StereotypeUtils.reapplyStereotypeAndGetTaggedValue(cla, metaProperty);
  ...
  private void setOrderProperty(VPContext context, IClass clickedClass) {
    final ITaggedValue baseTaggedValue =
        StereotypeUtils.reapplyStereotypeAndGetTaggedValue(
            clickedClass, StereotypeUtils.PROPERTY_ORDER);
    ...
    forEachSelectedClass(
        ...
        cla -> {
          ITaggedValue taggedValue =
              StereotypeUtils.reapplyStereotypeAndGetTaggedValue(
                  cla, StereotypeUtils.PROPERTY_ORDER);
