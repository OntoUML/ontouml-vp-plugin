...
public final class Relation extends Classifier<Relation, RelationStereotype> {
   ...
   public Relation(String id, MultilingualText name, RelationStereotype ontoumlStereotype, Classifier<?, ?> source, Classifier<?, ?> target) {
      this(id, name, ontoumlStereotype);
      createMemberEnd(source);
      createMemberEnd(target);
   }

   public Relation(String id, MultilingualText name, String stereotypeName, Classifier<?, ?> source, Classifier<?, ?> target) {
      this(id, name, stereotypeName);
      createMemberEnd(source);
      createMemberEnd(target);
   }

   public Relation(String id, MultilingualText name, RelationStereotype ontoumlStereotype, List<Classifier<?, ?>> participants) {
      this(id, name, ontoumlStereotype);
      participants.forEach(this::createMemberEnd);
   }

   public Relation(String id, MultilingualText name, String stereotypeName, List<Classifier<?, ?>> participants) {
      this(id, name, stereotypeName);
      participants.forEach(this::createMemberEnd);
   }

   public Relation(String id, String name, RelationStereotype ontoumlStereotype, Classifier<?, ?> source, Classifier<?, ?> target) {
   ...
   public void setStereotype(String stereotypeName) {
      ...
      stereotype.ifPresentOrElse(
              s -> setOntoumlStereotype(stereotype.get()),
              () -> setCustomStereotype(stereotypeName));
   ...
   public Property getDerivedRelationEnd() {
      if (!fromRelationToClass())
         throw new IllegalCallerException("Can only retrieve the derived relation end of a derivation relation.");
   ...
   public Property getDerivingClassEnd() {
      if (!fromRelationToClass())
         throw new IllegalCallerException("Can only retrieve the deriving class end of a derivation relation.");
   ...
   public Classifier<?, ?> getSource() {
      ...
      if (source.isEmpty())
         throw new NullPointerException("Source of the relation is null.");
   ...
   public Classifier<?, ?> getTarget() {
      ...
      if (target.isEmpty())
         throw new NullPointerException("Target of the relation is null.");
   ...
   public Relation getDerivedRelation() {
      ...
      if (relation.isEmpty())
         throw new NullPointerException("The derived relation is null.");
   ...
   public Class getDerivingClass() {
      ...
      if (!(_class.get() instanceof Class))
         throw new IllegalCallerException("The element from which the relation is derived is not a class.");
   ...
   public boolean involvesOnlyClasses() {
      ...
      return properties
              .stream()
              .allMatch(Property::isPropertyTypeClass);
   }
   ...
   public boolean fromRelationToClass() {
      return isBinary() &&
              getSource() instanceof Relation &&
              getTarget() instanceof Class;
   }
   ...
   public boolean isExternalDependence() {
      return isBinary() && getOntoumlStereotype().orElse(null) == RelationStereotype.EXTERNAL_DEPENDENCE;
   }
   ...
   public boolean propertiesSatisfy(List<Predicate<Property>> conditions) {
      if (conditions == null)
         throw new NullPointerException("Conditions list cannot be null.");

      if (arity() != conditions.size()) {
         throw new IllegalArgumentException("The number of conditions must be the same as that of properties of the relation.");
      }

      for (int i = 0; i < conditions.size(); i++) {
         if (!conditions.get(i).test(properties.get(i)))
            return false;
      }
   ...
   public boolean holdsBetweenEvents() {
      ...
      return properties
              .stream()
              .allMatch(p ->
                      p.isPropertyTypeClass() && p.getPropertyTypeAsClass().isRestrictedToEvents()
              );
   }
   ...
   public boolean holdsBetweenMoments() {
      ...
      return properties
              .stream()
              .allMatch(p -> p.isPropertyTypeClass() && p.getPropertyTypeAsClass().isRestrictedToMoments());
   }
   ...
   public boolean holdsBetweenSubstantials() {
      ...
      return properties
              .stream()
              .allMatch(p -> p.isPropertyTypeClass() && p.getPropertyTypeAsClass().isRestrictedToSubstantials());
   }
   ...
   public Property getCharacterizerEnd() {
      if (!isCharacterization())
         throw new IllegalCallerException("Can only retrieve characterizer (.e.g Mode, Quality) end of a characterization!");
   ...
   public Property getDependeeEnd() {
      if (!isExternalDependence())
         throw new IllegalCallerException("Can only retrieve dependee end of an external dependence relation!");
   ...
   public Property getDependerEnd() {
      if (!isExternalDependence())
         throw new IllegalCallerException("Can only retrieve depender end of an external dependence relation!");
   ...
   public Property getHistoricalDependeeEnd() {
      if (!isHistoricalDependence())
         throw new IllegalCallerException("Can only retrieve historical dependee end of a historical dependence relation!");
   ...
   public Property getHistoricalDependerEnd() {
      if (!isExternalDependence())
         throw new IllegalCallerException("Can only retrieve historical depender end of an historical dependence relation!");
   ...
   public Property getParticipantEnd() {
      if (!isParticipation())
         throw new IllegalCallerException("Can only retrieve participant end of a participation relation!");
   ...
   public Property getParticipationEnd() {
      if (!isParticipation())
         throw new IllegalCallerException("Can only retrieve participation (aka event) end of a participation relation!");
   ...
   public Property getCreationEventEnd() {
      if (!isCreation())
         throw new IllegalCallerException("Can only retrieve creation event end of a creation relation!");
   ...
   public Property getParticipationalWholeEnd() {
      if (!isParticipational())
         throw new IllegalCallerException("Can only retrieve participational whole end of a participational part-whole relation!");
   ...
   public Property getParticipationalPartEnd() {
      if (!isParticipational())
         throw new IllegalCallerException("Can only retrieve participational part end of a participational part-whole relation!");
   ...
   public Property getCategorizedEnd() {
      if (!isInstantiation())
         throw new IllegalCallerException("Can only retrieve categorized end of an instantiation relation!");
   ...
   public Property getCategorizerEnd() {
      if (!isInstantiation())
         throw new IllegalCallerException("Can only retrieve categorizer end of an instantiation relation!");
