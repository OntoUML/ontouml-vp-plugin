...
public class SmartColoring {
  ...
  public static void paintAll() {
    ...
    final IModelElement[] modelElements =
  ...
  private static Color getColor(IClass _class) {
    ...
    final Set<String> ultimateSortalStereotypes =
        StereotypeUtils.getUltimateSortalStereotypeNames();

    switch (restrictedTo) {
      case StereotypeUtils.RESTRICTED_TO_ABSTRACT:
        return COLOR_FOR_ABSTRACT;
      case StereotypeUtils.RESTRICTED_TO_COLLECTIVE:
        return ultimateSortalStereotypes.contains(stereotype)
            ? COLOR_FOR_COLLECTIVE_DARK
            : COLOR_FOR_COLLECTIVE;
      ...
      case StereotypeUtils.RESTRICTED_TO_FUNCTIONAL_COMPLEX:
        return ultimateSortalStereotypes.contains(stereotype)
            ? COLOR_FOR_FUNCTIONAL_COMPLEX_DARK
            : COLOR_FOR_FUNCTIONAL_COMPLEX;
      case StereotypeUtils.RESTRICTED_TO_QUALITY:
        return ultimateSortalStereotypes.contains(stereotype)
            ? COLOR_FOR_QUALITY_DARK
            : COLOR_FOR_QUALITY;
      case StereotypeUtils.RESTRICTED_TO_QUANTITY:
        return ultimateSortalStereotypes.contains(stereotype)
            ? COLOR_FOR_QUANTITY_DARK
            : COLOR_FOR_QUANTITY;
      case StereotypeUtils.RESTRICTED_TO_RELATOR:
        return ultimateSortalStereotypes.contains(stereotype)
            ? COLOR_FOR_RELATOR_DARK
            : COLOR_FOR_RELATOR;
      case StereotypeUtils.RESTRICTED_TO_TYPE:
        return COLOR_FOR_TYPE;
    }

    final List<String> restrictedToList =
        Arrays.stream(restrictedTo.split("\\s+")).map(s -> s.trim()).collect(Collectors.toList());

    final List<String> objectNatures =
        Arrays.asList(
    ...
    final List<String> intrinsicNature =
        Arrays.asList(StereotypeUtils.RESTRICTED_TO_MODE, StereotypeUtils.RESTRICTED_TO_QUALITY);
