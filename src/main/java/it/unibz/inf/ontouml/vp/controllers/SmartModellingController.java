   ...
   public static void setAssociationMetaProperties(IAssociation association) {
      ...
      switch (stereotype) {
         ...
         case Stereotype.MATERIAL:
            if (targetStereotype.equals(Stereotype.ROLE) || targetStereotype.equals(Stereotype.ROLE_MIXIN))
               setCardinalityIfEmpty(source, "1..*");
            else
               setCardinalityIfEmpty(source, "0..*");

            if (sourceStereotype.equals(Stereotype.ROLE) || sourceStereotype.equals(Stereotype.ROLE_MIXIN))
               setCardinalityIfEmpty(target, "1..*");
            else
               setCardinalityIfEmpty(target, "0..*");
...
//		final ArrayList<String> allowedCombinations = AssociationConstraints.allowedCombinations.get(new SimpleEntry<String, String>(sourceStereotype, targetStereotype));
      final List<String> allowedCombinations = OntoUMLConstraintsManager.getAllowedActionsBasedOnSourceTargetStereotypes(sourceStereotype, targetStereotype);
   ...
   public static void manageClassStereotypes(IClass _class, VPAction action) {
      ...
      for (int i = 0; relationshipsTo != null && i < relationshipsTo.length; i++) {
         ...
         final List<String> allowedCombinationsSub = OntoUMLConstraintsManager.getAllowedActionsBasedOnParentStereotype(superStereotype);
      ...
      for (int i = 0; relationshipsFrom != null && i < relationshipsFrom.length; i++) {
         ...
         final List<String> allowedCombinationsSuper = OntoUMLConstraintsManager.getAllowedActionsBasedOnChildStereotype(subStereotype);
