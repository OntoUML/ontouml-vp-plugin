...
/**
 * Controller responsible for opening the OntoUML Plugin configurations menu. <code>
 * OpenConfigurationsAction</code> contains an implementation of <code>IDialogHandler</code>
 * necessary for presenting Java components in Visual Paradigm.
 *
...
public class SettingsMenuController implements VPActionController {
  ...
  /**
   * Opens OntoUML Plugin configuration menu. Called when the button is clicked, providing access to
  ...
  public void performAction(VPAction action) {
    if (OntoUMLPlugin.getConfigWindowOpen() == true) return;
    else OntoUMLPlugin.setConfigWindowOpen(true);
  ...
  /**
   * Implementation of <code>IDialogHandler</code> necessary for presenting Java components in
   * Visual Paradigm.
   *
  ...
  protected class ConfigurationsDialog implements IDialogHandler {
    ...
    public Component getComponent() {
      _configurationsMenuView =
          new ConfigurationsView(Configurations.getInstance().getProjectConfigurations());
