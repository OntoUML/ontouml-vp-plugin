...
public class OBDAExportServerReques extends Thread{
	...
	private final ProjectConfigurations projectConfigurations = Configurations.getInstance().getProjectConfigurations();
	...
	public void run() {
		...
		try {
			...
			ApplicationManager.instance().getViewManager().showDialog(loading);//This class must extend Thread for the ProgressDialog to be shown properly.

			elements = Configurations.getInstance().getProjectConfigurations().getExportGUFOElementsDiagramTree();

			final BufferedReader buffer = OntoUMLServerAccessController.generateODBAFile(
			...
			if (buffer != null) {
				...
				ViewManagerUtils.cleanAndShowMessage("Unable to generate the OBDA mapping. Please check your model and parameters.");
			}
	...
	private void saveFile(BufferedReader buffer) throws IOException {
        ...
        String suggestedFileName = ApplicationManager.instance().getProjectManager().getProject().getName();
        String fileName;
        ...
        if (fd.getDirectory() != null && fd.getFile() != null) {
        	...
        	if( projectConfigurations.isGenerateConnection() ) {
        		Files.write(Paths.get(fd.getDirectory(), fileName+".properties"), result.getConnection().getBytes() );
        	}
        	
	        Files.write(Paths.get(fd.getDirectory(), fileName+".obda"), result.getObdaFile().getBytes() );
